...
class BotHelpOutput(OutputInfo):
    def make_output(self):
        result = '\nBot commands:\n{0}\nBook commands:\n{1}\nNotes commands:\n{2}\n'.format \
            (", ".join([key for key in BotCommands(Bot).exit_commands | BotCommands(Bot).help_commands]), \
...
class Bot:
    ...
    def answer_func(self, query:str):
        ...
        try:
            if not params:
                return eval(self.commands[command])()
            else:
                args = params.split(' ')
                return eval(self.commands[command])(*args)
        except KeyError:
